...
<title><?= $pageTitle ?? 'Student' ?> - EduManage</title>
...
<aside class="w-64 bg-white dark:bg-slate-900 border-r border-slate-200 dark:border-slate-800 flex flex-col sticky top-0 h-screen shrink-0 z-30">
  <a href="/CMS/student/dashboard.php" class="p-6 flex items-center hover:opacity-80 transition-opacity cursor-pointer">
    <img src="/CMS/assets/images/logo.png" alt="Logo" class="h-14 w-auto object-contain">
  </a>
  <nav class="flex-1 px-4 space-y-1">
    <?php
    $navItems = [
      ['href'=>'/CMS/student/dashboard.php',  'icon'=>'dashboard', 'label'=>'Dashboard'],
      ['href'=>'/CMS/student/my_courses.php', 'icon'=>'book_2',    'label'=>'My Courses'],
      ['href'=>'/CMS/student/catalog.php',    'icon'=>'explore',   'label'=>'Browse Courses'],
    ];
    foreach ($navItems as $item):
      $isActive = (basename($item['href']) === basename($_SERVER['PHP_SELF']));
      $cls = $isActive ? 'flex items-center gap-3 px-3 py-2.5 rounded-lg bg-primary/10 text-primary font-semibold'
  ...
  <div class="p-4 border-t border-slate-200 dark:border-slate-800">
    <div class="flex items-center gap-3">
      <div class="w-9 h-9 rounded-full bg-primary/10 flex items-center justify-center text-primary font-bold text-sm border border-primary/20">
        <?= strtoupper(substr($_SESSION['user_name']??'S',0,2)) ?>
      </div>
      ...
      <a href="/CMS/auth/logout.php" title="Logout" class="flex items-center justify-center">
        <span class="material-symbols-outlined text-slate-400 hover:text-red-500 cursor-pointer text-lg transition-colors">logout</span>
      </a>
    </div>
  </div>
</aside>
